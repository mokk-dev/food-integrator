...
class ReconciliationService:
    ...
    async def run_reconciliation_for_shift(
        self,
        merchant_id: str,
        opened_at: datetime,
        closed_at: datetime,
        ...
        try:
            ...
            if not api_order_ids:
                ...
                async with get_db_session() as session:
                    ...
                    result = await session.execute(
                        query, {"ids": [int(i) for i in api_order_ids]}
                    )
                ...
                if not missing_ids:
                    logger.info(
                        "reconciliation.orders_success",
                        msg="100% de conformidade de pedidos. Nenhum pedido perdido no turno.",
                    )
                else:
                    logger.warning(
                        "reconciliation.missing_orders_found",
            ...
            if shift_id:
                logger.info(
                    "reconciliation.cash_flow_started",
                    msg="Iniciando resgate de dados financeiros do caixa...",
                )
                await self._recover_cash_flow_data(shift_id, opened_at)
            else:
                logger.warning(
                    "reconciliation.cash_flow_skipped",
                    msg="ID do expediente não fornecido. Pulando auditoria financeira.",
                )

            logger.info(
                "reconciliation.completed",
                msg="Rotina completa de reconciliação de turno finalizada.",
            )

        except Exception as e:
            logger.error("reconciliation.failed", error=str(e))

    async def _fetch_history_with_rate_limit(
        ...
        while True:
            if page > 1:
                logger.debug(
                    "reconciliation.rate_limit",
                    msg=f"Aguardando {self.history_sleep_time}s (Rate Limit)...",
                )
                await asyncio.sleep(self.history_sleep_time)

            response = await self.public_api.get_orders_history_page(
    ...
    async def _recover_and_save_order(self, order_id: str):
        ...
        try:
            ...
            logger.error(
                "reconciliation.recover_failed", order_id=order_id, error=str(e)
            )

    async def _recover_missing_delivery_info(
        self, merchant_id: str, opened_at: datetime, closed_at: datetime
    ):
        """
        ...
        try:
            logger.info(
                "reconciliation.delivery_info_started",
                msg="Comparando contagem de motoboys (API vs Banco de Dados)...",
            )

            summary = await self.dashboard_api.get_delivery_men_summary(
                opened_at, closed_at
            )

            if not summary:
                logger.info(
                    "reconciliation.no_delivery_men",
                    msg="Nenhum motoboy com entregas no resumo da API.",
                )
                return
            ...
            for driver in summary:
                ...
                if api_total > 0 and api_total != db_total:
                    ...
                    orders = await self.dashboard_api.get_orders_by_delivery_man(
                        driver.get("id"), opened_at, closed_at
                    )
                        ...
                        order_updates.append(
                            {
                                ...
                                if driver_id_str and driver_id_str.isdigit()
                                else None,
            ...
            if order_updates:
                async with get_db_session() as session:
                    # ========================================================
                    # 📸 SNAPSHOT DE AUDITORIA DE MOTOBOYS (RAIO-X)
                    # Ocorre APENAS se houver divergência real a ser corrigida.
                    # ========================================================

                    # 1. Pega a foto exata de TODOS os pedidos de delivery no banco agora
                    snapshot_query = text("""
                        SELECT id, delivery_man_id, delivery_man_name, status 
                        FROM orders 
                        WHERE merchant_id = :mid 
                          AND order_type = 'delivery'
                          AND created_at >= :opened_at 
                    ...
                    current_db_state = [
                        {
                            "order_id": row[0],
                            "driver_id_banco": row[1],
                            "driver_name_banco": row[2],
                            "status_banco": row[3],
                        }
                        for row in result.fetchall()
                    ]

                    # 2. Loga o cenário completo do Banco (O que temos)
                    logger.warning(
                        "reconciliation.delivery_audit_snapshot_before",
                        msg="AUDITORIA MOTOBOYS: Estado de todas as ordens de delivery ANTES da correção",
                        total_ordens_delivery_no_banco=len(current_db_state),
                        banco_state=current_db_state,
                    )

                    # 3. Loga as correções exatas ditadas pela API (Para onde vamos)
                    logger.warning(
                        "reconciliation.delivery_audit_planned_changes",
                        msg="AUDITORIA MOTOBOYS: Lista de ordens que SERÃO SOBRESCRITAS com os dados da API",
                        total_alteracoes=len(order_updates),
                        api_state=order_updates,
                    )
                    # ========================================================

                    # Executa as alterações de fato
                    for update in order_updates:
                        update_query = text("""
                            UPDATE orders SET 
                                delivery_man_id = :driver_id,
                ...
                logger.info(
                    "reconciliation.delivery_info_synced",
                    msg="100% de conformidade! A contagem de todos os motoboys bateu perfeitamente.",
                )
    ...
    async def _recover_cash_flow_data(self, shift_id: int, opened_at: datetime):
        ...
        """
        try:
            recent_cash_flows = await self.dashboard_api.get_cash_flows(
                page=1, per_page=10
            )
            ...
            for cf in recent_cash_flows:
                ...
                cf_open_at = datetime.fromisoformat(
                    cf.get("open_at").replace("Z", "+00:00")
                )
            ...
            if not target_cash_flow:
                logger.warning(
                    "reconciliation.cash_flow_not_found",
                    shift_id=shift_id,
                    msg="Nenhum caixa da API corresponde a este expediente.",
                )
                return
            ...
            async with get_db_session() as session:
                ...
                await session.commit()
                logger.info(
                    "reconciliation.cash_flow_success",
                    msg="Dados financeiros do caixa gravados com sucesso.",
                )
