    ...
    async def refresh_tokens(self) -> str:
        ...
        async with self._auth_lock:
            ...
            if not refresh_token:
                # Se não tem no Redis, busca a fonte da verdade no banco
                async with get_db_session() as session:
                    result = await session.execute(
                        text("SELECT refresh_token FROM merchant_credentials WHERE merchant_id = :mid AND auth_status = 'ACTIVE'"),
            ...
            if not refresh_token:
                raise Exception("Nenhum Refresh Token ativo no Redis, Banco ou .env. Necessária injeção manual.")
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code != 200:
                    ...
                    async with get_db_session() as session:
                        await session.execute(
                            text("UPDATE merchant_credentials SET auth_status = 'EXPIRED', updated_at = NOW() WHERE merchant_id = :mid"),
                            {"mid": str(settings.default_merchant_id)}
                        )
                    
                    raise Exception(f"Falha fatal ao renovar tokens. Cadeia de Refresh expirou. É necessário injetar credenciais manualmente. Log: {response.text}")
                ...
                logger.info("auth.tokens_refreshed", msg="Tokens renovados no PostgreSQL e Redis com sucesso.")
                return new_access